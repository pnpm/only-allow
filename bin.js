...
const wantedPMs = argv;
wantedPMs.forEach((wantedPM) => {
  if (
    wantedPM !== "npm" &&
    wantedPM !== "cnpm" &&
    wantedPM !== "pnpm" &&
    ...
    console.log(
      `"${wantedPM}" is not a valid package manager. Available package managers are: npm, cnpm, pnpm, yarn or bun.`
    );
    process.exit(1);
  }
});
...
if (usedPM && !wantedPMs.includes(usedPM.name) && !isInstalledAsDependency) {
  switch (wantedPMs[0]) {
    case "npm":
